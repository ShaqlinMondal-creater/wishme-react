...
export function WishLetterRoom({ onBack }: WishLetterRoomProps) {
  ...
  const letter = editor?.content.rooms.letter
  const opened = phase === 'opened' || Boolean(editor?.enabled)
  const greeting = letter?.greeting ?? wishLetter.greeting
  const body = letter?.body ?? wishLetter.body
  const signoff = letter?.signoff ?? wishLetter.signoff
  const date = letter?.date ?? wishLetter.date
  const heading = letter?.heading ?? 'For Riya, in his hand'
  const intro = letter?.intro ?? 'A page he would not send in a chat. Open it once, the way you would a letter left on the table.'
  const cover = letter?.cover

  const openLetter = () => {
    if (phase !== 'sealed' || editor?.enabled) {
      return
    }
  ...
  return (
    <RoomFrame kicker={opened ? 'Opened' : 'Sealed'} title={letter?.title ?? 'Letter'} onBack={onBack}>
      {editor?.enabled ? (
        <div className="relative mb-4 h-28 overflow-hidden rounded-[1.2rem]">
          {cover ? <WishMedia src={cover} className="h-full w-full object-cover" /> : <div className="h-full bg-navy" />}
          <EditPencil
            label="Letter cover"
            onClick={() => editor.onEdit('rooms.letter.cover', 'Letter cover', 'media')}
          />
          <div className="absolute inset-0 bg-linear-to-t from-black/70 to-transparent" />
          <div className="absolute right-12 bottom-2 left-3">
            <p className="font-display text-xl text-white">{heading}</p>
            <p className="mt-1 line-clamp-2 text-xs text-gold-soft">{intro}</p>
            <div className="mt-2 flex flex-wrap gap-2">
              <button
                type="button"
                className="rounded-full bg-gold px-2 py-0.5 text-[10px] text-navy"
                onClick={() => editor.onEdit('rooms.letter.heading', 'Letter heading', 'text')}
              >
                Heading
              </button>
              <button
                type="button"
                className="rounded-full bg-gold px-2 py-0.5 text-[10px] text-navy"
                onClick={() => editor.onEdit('rooms.letter.intro', 'Letter intro', 'textarea')}
              >
                Intro
              </button>
            </div>
      ...
      {!opened ? (
        <div className="grid h-full min-h-[min(28rem,calc(100svh-8rem))] items-center gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:gap-12">
          ...
          >
            <div className={cn('wish-env', phase === 'opening' && 'is-opening')}>
              <div className="wish-env-body relative aspect-[5/3] w-full rounded-[0.2rem] shadow-[0_28px_60px_-28px_rgb(0_0_0_/_0.75)]">
                {cover ? <WishMedia src={cover} className="absolute inset-0 h-full w-full object-cover opacity-35" /> : null}
                ...
                <div className="wish-env-sheet absolute inset-x-[11%] top-[22%] bottom-[10%] overflow-hidden rounded-[0.12rem] bg-[#f4eee4] shadow-inner">
                  <p className="px-4 pt-4 font-display text-lg italic text-navy/70 sm:text-xl">{greeting}</p>
                </div>
          ...
          <div className="relative mx-auto max-w-md text-center lg:mx-0 lg:text-left">
            <p className="text-[11px] tracking-[0.28em] text-gold uppercase">{date}</p>
            <h2 className="mt-3 font-display text-3xl text-white sm:text-4xl">{heading}</h2>
            <p className="mt-3 text-sm leading-7 text-gold-soft">{intro}</p>
            <button
              type="button"
        ...
        <article className="wish-paper-lift wish-paper relative mx-auto mt-2 max-w-2xl overflow-hidden rounded-[0.35rem] px-6 py-8 text-navy shadow-[0_20px_50px_-28px_rgb(0_0_0_/_0.65)] sm:px-10 sm:py-12 lg:mt-6">
          ...
          <div className="relative">
            <div className="flex items-start justify-between gap-4">
              <p className="text-[10px] tracking-[0.22em] text-gold-deep uppercase">{date}</p>
              <WaxSeal className="h-10 w-10 opacity-90" />
            </div>
            {editor?.enabled ? (
              <EditPencil label="Date" onClick={() => editor.onEdit('rooms.letter.date', 'Letter date', 'text')} />
            ) : null}
            <p className="mt-6 font-display text-[2rem] italic">{greeting}</p>
            {editor?.enabled ? (
              <EditPencil
                label="Greeting"
                className="relative mt-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-gold text-navy"
                onClick={() => editor.onEdit('rooms.letter.greeting', 'Greeting', 'text')}
              />
            ) : null}
            {body.map((paragraph) => (
            ...
            {editor?.enabled ? (
              <EditPencil
                label="Letter body"
                className="relative mt-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-gold text-navy"
                onClick={() => editor.onEdit('rooms.letter.body', 'Letter body', 'lines')}
              />
            ) : null}
            <p className="mt-10 font-display text-lg leading-8 italic whitespace-pre-line text-gold-deep">{signoff}</p>
            {editor?.enabled ? (
              <EditPencil
                label="Sign off"
                className="relative mt-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-gold text-navy"
                onClick={() => editor.onEdit('rooms.letter.signoff', 'Sign off', 'textarea')}
              />
            ) : null}
          </div>
        </article>
