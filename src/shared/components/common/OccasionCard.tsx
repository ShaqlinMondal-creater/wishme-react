...
export function OccasionCard({ occasion, to, actions, layout = 'default' }: OccasionCardProps) {
    ...
    <Card padding="none" hover className="h-full overflow-hidden">
      <div className="relative">
        <img
          ...
          alt=""
          className={isCover ? 'h-56 w-full object-cover sm:h-72' : 'h-44 w-full object-cover sm:h-52'}
        />
        {isCover ? (
        ...
        {isCover ? (
          <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
            <p className="text-[10px] tracking-[0.22em] text-gold uppercase">{occasionTypeLabel(occasion.type)}</p>
            <h3 className="mt-2 font-display text-3xl text-white">{occasion.title}</h3>
          </div>
        ) : null}
        {actions ? (
      ...
      {isCover ? (
        <div className="p-5 sm:p-6">
          <p className="line-clamp-2 text-sm leading-6 text-navy-muted">{occasion.description}</p>
        </div>
      ) : (
        <div className="p-5 sm:p-6">
